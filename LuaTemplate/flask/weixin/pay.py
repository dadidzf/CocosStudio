...
class WxPayConf_pub(object):
    ...
    APPID = "wxfdd043a757fb8ecb"
    #JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    APPSECRET = "b131a89d8fcf5c422b475e5aa5f006b3"
    #受理商ID，身份标识
    MCHID = "1500649171"
    #商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    KEY = "changshashiyingxiongyongwukeji66"
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def __init__(self, timeout=WxPayConf_pub.CURL_TIMEOUT):
        #设置接口链接
        print('UnifiedOrder_pub __init__')
    ...
    def createXml(self):
        ...
        ret = self.arrayToXml(self.parameters)
        print ret
        return ret

    def createRetJson(self, prepayid):
        ret = {}
        ret["appid"] = WxPayConf_pub.APPID
        ret["partnerid"] = WxPayConf_pub.MCHID
        ret["prepayid"] = prepayid
        ret["package"] = "Sign=WXPay"
        ret["noncestr"] = self.createNoncestr()
        ret["timestamp"] = str((int)(time.time()))
        ret["sign"] = self.getSign(ret)   #签名
