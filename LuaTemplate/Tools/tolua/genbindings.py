...
def main():
    ...
    if not os.path.exists(x64_llvm_path):
        x64_llvm_path = os.path.abspath(os.path.join(ndk_root, 'toolchains/llvm/prebuilt', 'darwin-x86_64'))
    ...
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'frameworks', 'runtime-src'))
    ...
    try:
        ...
        tolua_root = os.path.abspath(os.path.dirname(__file__))
        output_dir = '%s/Classes/LuaUserBindings' % project_root

        cmd_args = {'bindings.ini' : ('scripting', 'lua_scripting_auto')}
