...
def relplaceGameConfig(name, packageName):
	curPath = get_current_path()
	replace_string(os.path.join(curPath, "src", "app", name, "GameConfig.json"), '"appName":"template"', 
		'"appName":"%s"'%name)
	replace_string(os.path.join(curPath, "src", "app", name, "GameConfig.json"), '"packageName":"com.yongwu.luatemplate",', 
...
def applayGameConfigToProject(name):
	...
	replaceRegularString(os.path.join(curPath, "frameworks/runtime-src/proj.android-studio/app/google-services.json"),
		'"package_name":.*', '"package_name": "%s"'%gameConfig['android']['packageName'])
	f.close()
